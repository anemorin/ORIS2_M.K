  ...
  return (
    <Container>
      ...
      <PokemonData>
        <PokemonDataItem>
          {pokemon.stats
            .filter((pok) => !['special-attack', 'special-defense'].includes(pok.stat.name))
            .map((stat, index) => (
            <div key={index}>
              <p>{toUpperCase(stat.stat.name)}</p>
              <PokemonDataItemBar
        ...
        <PokemonImage src={pokemon.sprites.other?.home.front_default?? pokemon.sprites.front_default} />
      </PokemonData>
    </Container>
  )
}
